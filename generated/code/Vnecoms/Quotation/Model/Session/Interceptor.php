<?php
namespace Vnecoms\Quotation\Model\Session;

/**
 * Interceptor class for @see \Vnecoms\Quotation\Model\Session
 */
class Interceptor extends \Vnecoms\Quotation\Model\Session implements \Magento\Framework\Interception\InterceptorInterface
{
    use \Magento\Framework\Interception\Interceptor;

    public function __construct(\Magento\Framework\App\Request\Http $request, \Magento\Framework\Session\SidResolverInterface $sidResolver, \Magento\Framework\Session\Config\ConfigInterface $sessionConfig, \Magento\Framework\Session\SaveHandlerInterface $saveHandler, \Magento\Framework\Session\ValidatorInterface $validator, \Magento\Framework\Session\StorageInterface $storage, \Magento\Framework\Stdlib\CookieManagerInterface $cookieManager, \Magento\Framework\Stdlib\Cookie\CookieMetadataFactory $cookieMetadataFactory, \Magento\Framework\App\State $appState, \Magento\Customer\Model\Session $customerSession, \Vnecoms\Quotation\Api\QuoteRepositoryInterface $quoteRepository, \Magento\Framework\HTTP\PhpEnvironment\RemoteAddress $remoteAddress, \Magento\Framework\Event\ManagerInterface $eventManager, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Customer\Api\CustomerRepositoryInterface $customerRepository, \Vnecoms\Quotation\Model\QuoteFactory $quoteFactory)
    {
        $this->___init();
        parent::__construct($request, $sidResolver, $sessionConfig, $saveHandler, $validator, $storage, $cookieManager, $cookieMetadataFactory, $appState, $customerSession, $quoteRepository, $remoteAddress, $eventManager, $storeManager, $customerRepository, $quoteFactory);
    }
    ...
    public function addProduct(\Magento\Catalog\Model\Product $product, $params)
    {
        ...
        if (!$pluginInfo) {
            return parent::addProduct($product, $params);
        } else {
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'isGuestQuote');
        if (!$pluginInfo) {
            return parent::isGuestQuote();
        } else {
            return $this->___callPlugins('isGuestQuote', func_get_args(), $pluginInfo);
        }
    }
