...
class Interceptor extends \MSP\ReCaptcha\Block\Frontend\ReCaptcha implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    public function __construct(\Magento\Framework\View\Element\Template\Context $context, \Magento\Framework\Json\DecoderInterface $decoder, \Magento\Framework\Json\EncoderInterface $encoder, \MSP\ReCaptcha\Model\LayoutSettings $layoutSettings, array $data = [])
    {
        $this->___init();
        parent::__construct($context, $decoder, $encoder, $layoutSettings, $data);
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'getPublicKey');
        if (!$pluginInfo) {
            return parent::getPublicKey();
        } else {
            return $this->___callPlugins('getPublicKey', func_get_args(), $pluginInfo);
        }
    }
