...
class Interceptor extends \Mirasvit\Search\Block\Adminhtml\Menu implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    public function addItem($data, $parent = null)
    {
        ...
        if (!$pluginInfo) {
            return parent::addItem($data, $parent);
        } else {
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'addSeparator');
        if (!$pluginInfo) {
            return parent::addSeparator();
        } else {
            return $this->___callPlugins('addSeparator', func_get_args(), $pluginInfo);
        }
    }
